...
const ProfilePage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      format: row => new Date(row.bookingDate).toLocaleDateString('en-GB')
    },
    {
      name: 'Pickup Date',
      selector: 'pickupDate',
      sortable: true,
      format: row => new Date(row.pickupDate).toLocaleDateString('en-GB')
    },
    {
      name: 'Dropoff Date',
      selector: 'dropoffDate',
      sortable: true,
      format: row => new Date(row.dropoffDate).toLocaleDateString('en-GB')
    },
    {
      name: 'Total Amount',
      ...
      cell: row => (
        <div>
          {row.status === 'Cancelled' ? (
            <span className="text-red-500">Booking Cancelled</span>
          ) : new Date(row.pickupDate) > new Date() ? (
            <button
              className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded"
              onClick={() => handleCancel(row._id, row.pickupDate)}
            >
              Cancel
            </button>
          ) : (
